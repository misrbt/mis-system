...
class DashboardController extends Controller
{
    ...
    /**
     * Get all assets currently with "Under Repair" status
     * Used for the reminder popup on inventory/dashboard access
     */
    public function getUnderRepairAssets()
    {
        try {
            $cacheKey = 'dashboard:under_repair_assets';

            $assets = Cache::remember($cacheKey, 120, function () {
                $now = now();
                ...
                return DB::table('assets')
                    ...
                    ->where('status.name', 'Under Repair')
                    ->orderBy('assets.updated_at', 'desc')
                    ->get()
                    ->map(function ($asset) use ($now) {
                        // Calculate days under repair (since last updated to this status)
                        $updatedAt = $asset->updated_at
                            ? \Carbon\Carbon::parse($asset->updated_at)
                            : $now;
                        $daysUnderRepair = $updatedAt->diffInDays($now);
                        ...
                        return [
                            ...
                            'branch' => $asset->branch_name ?? 'Unassigned',
                            'assigned_to' => trim($asset->employee_name) ?: 'Unassigned',
                            'days_under_repair' => $daysUnderRepair,
                            'since' => $updatedAt->format('Y-m-d'),
                        ];
                    });
            });
            ...
            return response()->json([
                'success' => true,
                'data' => [
                    'has_under_repair' => $assets->count() > 0,
                    'count' => $assets->count(),
                    'assets' => $assets,
                ],
            ], 200);
